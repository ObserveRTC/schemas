...
export class RecordEncoder {
	...
	private encodeStruct(
		...
		if (!encoder) {
			encoder = new RecordEncoder(field.plan);
	...
	private encodeValueList(
		...
		// in one sample.)
		const stateless = new RecordEncoder(field.plan);
	...
	private encodeEntityList(
		...
		for (let index = 0; index < items.length; index += 1) {
			...
			if (!encoder) {
				encoder = new RecordEncoder(field.plan, pinned);
		...
		for (const [slotKey, encoder] of slots) {
			if (encoder.visited) encoder.visited = false;
...
export class RecordDecoder {
	...
	public constructor(
		private readonly plan: MessagePlan,
		private readonly context: RecordDecoderContext,
	) {}
	...
	private decodeEntityList(
		...
		for (const [slotKey, decoder] of slots) {
			if (decoder.visited) decoder.visited = false;
			else slots.delete(slotKey);
		}
